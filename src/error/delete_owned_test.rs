//! Owned-deletion error regressions.
...
fn returned_error_exposes_the_exact_descriptor() -> Result<(), std::io::Error> {
    ...
    let error = DeleteOwnedError::returned(Error::Invariant, descriptor);
...
fn retained_error_exposes_the_exact_registration() {
    ...
    let error = DeleteOwnedError::retained(Error::Invariant, registration);
