//! Linux caller-arena phase transition regressions.

use std::panic::{AssertUnwindSafe, catch_unwind};

use crate::{Error, Event, Events, Key, Readiness};
...
fn unstamped_storage_never_exposes_an_event() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(matches!(
        batch.translate(&mut events, 1, None, |_| Ok(Some(Key::ZERO))),
        Err(Error::Invariant)
    ));
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    batch.translate(&mut events, observed, Some(Key::new(99)), |token| {
        Ok((token == 13).then_some(Key::new(93)))
    })?;
...
fn mismatched_stamp_is_rejected_and_invalidated() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(matches!(
        batch.translate(&mut replacement, observed, None, |_| Ok(Some(Key::ZERO))),
        Err(Error::Invariant)
    ));
    assert!(staged.is_empty());
    assert!(replacement.is_empty());
    assert!(matches!(
        batch.translate(&mut staged, observed, None, |_| Ok(Some(Key::ZERO))),
        Err(Error::Invariant)
    ));
    ...
    assert!(matches!(
        batch.translate(&mut staged, observed + 1, None, |_| Ok(Some(Key::ZERO))),
        Err(Error::Invariant)
    ));
    assert!(staged.is_empty());
    Ok(())
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(events.is_empty());
    assert!(matches!(
        batch.translate(&mut events, observed, None, |_| Ok(Some(Key::ZERO))),
        Err(Error::Invariant)
    ));
